export default function TimelineEvent({evnt_date, title, description, url}) {
    ...
    let evntDate = evnt_date.includes("BC") ? evnt_date : evnt_date + " AD";
    
    return (
        <li className="mb-10 ml-4">
            <div className="absolute w-3 h-3 rounded-full mt-1.5 -left-1.5 bg-orange border border-black"></div>
            {/* <time className="mb-1 text-sm font-normal leading-none text-gray">{evnt_date.includes("BC") ? date : date + " AD"}</time> */}
            <time className="mb-1 text-sm font-normal leading-none text-gray">{evntDate}</time>
