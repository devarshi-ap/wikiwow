export default function TimelineEvent({date, title, description, url}) {
    return (
        <li className="mb-10 ml-4">
            <div className="absolute w-3 h-3 rounded-full mt-1.5 -left-1.5 bg-orange border border-black"></div>
            <time className="mb-1 text-sm font-normal leading-none text-gray">{date.includes("BC") ? date : date + " AD"}</time>
            <h3 className="text-lg mb-2 font-semibold text-dark_gray">{title}</h3>
